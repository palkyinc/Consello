...
return new class extends Migration
{
    ...
    private function factoryPermissions()
    {
        $User = App\Models\User::find(1);
        $listRols = array('Admin');
        ...
        if (!empty($listRols)) {
            foreach ($listRols as $key => $listRol) {
                Spatie\Permission\Models\Role::create(['name' => $listRol]);
        ...
        $rolAdmin = Spatie\Permission\Models\Role::find(1);
        if (!empty($listPermissions)) {
            foreach ($listPermissions as $key => $listPermission) {
                $permission = Spatie\Permission\Models\Permission::create(['name' => $listPermission]);
